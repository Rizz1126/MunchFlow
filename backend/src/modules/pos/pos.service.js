...
export async function processSale(data, userId, businessId) {
  ...
  return await db.transaction(async (tx) => {
    ...
    for (const item of items) {
      ...
      const subtotal = (menu.sellingPrice + modifiersTotal) * item.quantity;
    ...
    const [sale] = await tx.insert(sales).values({ invoiceNumber, totalAmount, totalHpp, paymentMethod: paymentMethod || 'cash', createdBy: userId, businessId }).returning();
    await tx.insert(saleItems).values(processedItems.map((item) => ({ 
      saleId: sale.id, 
    ...
    await tx.insert(cashTransactions).values({ type: 'income', category: 'Penjualan Harian', amount: totalAmount, description: `Penjualan ${invoiceNumber}`, paymentMethod: paymentMethod || 'cash', relatedSaleId: sale.id, createdBy: userId, businessId, transactionDate: now.toISOString() });
    return { ...sale, items: processedItems };
  });
}

export async function getRecentSales(limit = 50, businessId, businessIds) {
  let condition = undefined;
  if (businessId) {
    condition = eq(sales.businessId, businessId);
  } else if (businessIds && businessIds.length > 0) {
    condition = inArray(sales.businessId, businessIds);
  }
  ...
  return await Promise.all(salesList.map(async (sale) => ({ ...sale, items: await db.select({ id: saleItems.id, menuItemId: saleItems.menuItemId,
    menuName: menuItems.name, quantity: saleItems.quantity, unitPrice: saleItems.unitPrice, 
    modifiersTotal: saleItems.modifiersTotal, note: saleItems.note,
    subtotal: saleItems.subtotal, hppPerItem: saleItems.hppPerItem })
