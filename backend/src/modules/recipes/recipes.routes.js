...
router.get('/', authenticate, (req, res, next) => {
  try {
    res.json(recipesService.getAllMenuItems());
  } catch (err) {
    next(err);
  }
});
...
router.get('/:id', authenticate, (req, res, next) => {
  try {
    const menu = recipesService.getMenuItemById(parseInt(req.params.id));
...
router.post('/', authenticate, requireRole('owner'), (req, res, next) => {
  try {
    ...
    const menu = recipesService.createMenuItem(req.body);
...
router.put('/:id', authenticate, requireRole('owner'), (req, res, next) => {
  try {
    const menu = recipesService.updateMenuItem(parseInt(req.params.id), req.body);
    if (!menu) return res.status(404).json({ error: 'Menu tidak ditemukan.' });
    res.json(menu);
  } catch (err) {
    next(err);
...
router.post('/:id/recipe', authenticate, requireRole('owner'), (req, res, next) => {
  try {
    const { items } = req.body; // [{ingredientId, quantityNeeded}]
    if (!items || !Array.isArray(items)) {
      return res.status(400).json({ error: 'Items resep wajib diisi (array).' });
    }
    const menu = recipesService.setRecipe(parseInt(req.params.id), items);
    res.json(menu);
  } catch (err) {
    next(err);
  }
...
// GET /api/menu/:id/hpp
router.get('/:id/hpp', authenticate, (req, res, next) => {
  try {
    const hpp = recipesService.calculateHpp(parseInt(req.params.id));
